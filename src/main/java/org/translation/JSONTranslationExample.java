...
public class JSONTranslationExample {
    ...
    public String getCountryNameTranslation(String countryCode, String languageCode) {
        for (int i = 0; i < jsonArray.length(); i++) {
            JSONObject objectAtLine = jsonArray.getJSONObject(i);
            String refactoredCountryCode = countryCode.toLowerCase();
            if (objectAtLine.getString("alpha3").equals(refactoredCountryCode)) {
                return objectAtLine.getString(languageCode);
            }
        }
